...
class phpthumb {
	...
	function phpThumb() {
        ...
        $this->phpthumb_functions = new phpthumb_functions();
	}
	...
	function setSourceData($rawImageData, $sourceFilename='') {
		...
		$this->DebugMessage('setSourceData() setting $this->rawImageData ('.strlen($this->rawImageData).' bytes; magic="'.substr($this->rawImageData, 0, 4).'" ('.$this->phpthumb_functions->HexCharDisplay(substr($this->rawImageData, 0, 4)).'))', __FILE__, __LINE__);
...
//		}
	...
	function OutputThumbnail() {
		...
		$downloadfilename = $this->phpthumb_functions->SanitizeFilename(is_string($this->sia) ? $this->sia : ($this->down ? $this->down : 'phpThumb_generated_thumbnail'.'.'.$this->thumbnailFormat));
		...
		if ($this->useRawIMoutput) {
			...
			header('Content-Type: '.$this->phpthumb_functions->ImageTypeToMIMEtype($this->thumbnailFormat));
	...
	function CleanUpCacheDirectory() {
		...
		if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
			...
			$AllFilesInCacheDirectory = $this->phpthumb_functions->GetAllFilesInSubfolders($this->config_cache_directory);
	...
	function setOutputFormat() {
		...
		if (strtolower($this->f) == 'jpg') {
			$this->f = 'jpeg';
		        ...
		        if ($this->phpthumb_functions->CaseInsensitiveInArray($this->config_output_format, $AvailableImageOutputFormats)) {
		...
		if ($this->f && ($this->phpthumb_functions->CaseInsensitiveInArray($this->f, $AvailableImageOutputFormats))) {
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (eregi('^[a-z0-9]+\:/{1,2}', $filename)) {
			...
			if (@is_readable($filename) && !@is_readable($this->config_document_root.$filename)) {
				...
				// /~user/path
				if ($ApacheLookupURIarray = $this->phpthumb_functions->ApacheLookupURIarray($filename)) {
					$AbsoluteFilename = $ApacheLookupURIarray['filename'];
				} else {
					$AbsoluteFilename = realpath($filename);
					if (@is_readable($AbsoluteFilename)) {
						$this->DebugMessage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.$filename.'", but the correct filename ('.$AbsoluteFilename.') seems to have been resolved with realpath($filename)', __FILE__, __LINE__);
					} elseif (is_dir(dirname($AbsoluteFilename))) {
						$this->DebugMessage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.dirname($filename).'", but the correct directory ('.dirname($AbsoluteFilename).') seems to have been resolved with realpath(.)', __FILE__, __LINE__);
					} else {
						return $this->ErrorImage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.$filename.'". This has been known to fail on Apache2 - try using the absolute filename for the source image (ex: "/home/user/httpdocs/image.jpg" instead of "/~user/image.jpg")');
					}
				}
			...
			if (substr(dirname(@$_SERVER['PHP_SELF']), 0, 2) == '/~') {
				if ($ApacheLookupURIarray = $this->phpthumb_functions->ApacheLookupURIarray(dirname(@$_SERVER['PHP_SELF']))) {
					...
					if (@is_readable($AbsoluteFilename)) {
						$this->DebugMessage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'", but the correct filename ('.$AbsoluteFilename.') seems to have been resolved with realpath(.)/$filename', __FILE__, __LINE__);
					} elseif (is_dir(dirname($AbsoluteFilename))) {
						$this->DebugMessage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'", but the correct directory ('.dirname($AbsoluteFilename).') seems to have been resolved with realpath(.)', __FILE__, __LINE__);
					} else {
						return $this->ErrorImage('$this->phpthumb_functions->ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'". This has been known to fail on Apache2 - try using the absolute filename for the source image');
					}
				}
			}
	...
	function ImageMagickWhichConvert() {
		...
		if (is_null($WhichConvert)) {
			if ($this->iswindows) {
		       ...
		       $WhichConvert = trim($this->phpthumb_functions->SafeExec('which convert'));
			}
		}
		return $WhichConvert;
	}
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				$versionstring[1] = trim($this->phpthumb_functions->SafeExec($commandline));
	...
	function ImageMagickSwitchAvailable($switchname) {
		...
		if (is_null($IMoptions)) {
			...
			if (!is_null($commandline)) {
				$commandline .= ' -help';
				$IMhelp_lines = explode("\n", $this->phpthumb_functions->SafeExec($commandline));
	...
	function ImageMagickFormatsList() {
		...
		if (is_null($IMformatsList)) {
			...
			if (!is_null($commandline)) {
				...
				$IMformatsList = $this->phpthumb_functions->SafeExec($commandline);
			}
		}
	...
	function ImageMagickThumbnailToGD() {
		...
		$this->useRawIMoutput = true;
        		if ($this->phpthumb_functions->gd_version()) {
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$IMresult_test = $this->phpthumb_functions->SafeExec($commandline_test);
				...
				if (is_array($getimagesize)) {
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							$side  = $this->phpthumb_functions->nonempty_min($this->source_width, $this->source_height, max($wAll, $hAll));
							$sideX = $this->phpthumb_functions->nonempty_min($this->source_width,                       $wAll, round($hAll * $zcAR));
							$sideY = $this->phpthumb_functions->nonempty_min(                     $this->source_height, $hAll, round($wAll / $zcAR));
							...
							if ($this->iar && (intval($this->w) > 0) && (intval($this->h) > 0)) {
								...
								$this->w = ((($this->aoe || $this->far) && $this->w) ? $this->w : ($this->w ? $this->phpthumb_functions->nonempty_min($this->w, $getimagesize[0]) : ''));
								$this->h = ((($this->aoe || $this->far) && $this->h) ? $this->h : ($this->h ? $this->phpthumb_functions->nonempty_min($this->h, $getimagesize[1]) : ''));
								if ($this->w || $this->h) {
									if ($IMuseExplicitImageOutputDimensions) {
										if ($this->w && !$this->h) {
											$this->h = ceil($this->w / ($this->source_width / $this->source_height));
						...
						case 'bord':
							if ($this->ImageMagickSwitchAvailable(array('border', 'bordercolor', 'thumbnail', 'crop'))) {
								if (!$this->zc) {
									...
									if ($width && !$rX && !$rY) {
										if (!$this->phpthumb_functions->IsHexColor($color)) {
				...
				$IMresult = $this->phpthumb_functions->SafeExec($commandline);
				clearstatcache();
	...
	function Rotate() {
        ...
        if ($this->ra || $this->ar) {
			...
			if (!$this->phpthumb_functions->IsHexColor($this->config_background_hexcolor)) {
			...
			if ($this->ra) {
				...
				if ($this->ar == 'x') {
					if ($this->phpthumb_functions->version_compare_replacement(phpversion(), '4.2.0', '>=')) {
						if ($this->sourceFilename) {
	...
	function AntiOffsiteLinking() {
		...
		if ($allow && $this->config_nooffsitelink_enabled && (@$_SERVER['HTTP_REFERER'] || $this->config_nooffsitelink_require_refer)) {
			...
			$parsed_url = $this->phpthumb_functions->ParseURLbetter(@$_SERVER['HTTP_REFERER']);
...
exit;
		...
		if ($allow && $this->config_nohotlink_enabled && eregi('^(f|ht)tps?\://', $this->src)) {
			$parsed_url = $this->phpthumb_functions->ParseURLbetter($this->src);
			//if (!$this->phpthumb_functions->CaseInsensitiveInArray(@$parsed_url['host'], $this->config_nohotlink_valid_domains)) {
		...
		if (!$this->phpthumb_functions->IsHexColor($this->config_error_bgcolor)) {
			return $this->ErrorImage('Invalid hex color string "'.$this->config_error_bgcolor.'" for $this->config_error_bgcolor');
		}
		if (!$this->phpthumb_functions->IsHexColor($this->config_error_textcolor)) {
		...
		if ($erase) {
			...
			if (is_file($this->config_nooffsitelink_watermark_src)) {
				...
				$nohotlink_text_color = $this->phpthumb_functions->ImageHexColorAllocate($this->gdimg_output, $this->config_error_textcolor);
	...
	function AlphaChannelFlatten() {
		...
		switch ($this->thumbnailFormat) {
			...
			case 'gif':
				...
				if ($CurrentImageColorTransparent == -1) {
					...
					if ($this->phpthumb_functions->gd_version() < 2.0) {
						$this->DebugMessage('AlphaChannelFlatten() failed because GD version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
						return false;
					}
					...
					if ($img_alpha_mixdown_dither = @ImageCreateTrueColor(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output))) {
						...
						for ($x = 0; $x < $this->thumbnail_width; $x++) {
							for ($y = 0; $y < $this->thumbnail_height; $y++) {
								$PixelColor = $this->phpthumb_functions->GetPixelColor($this->gdimg_output, $x, $y);
						...
						for ($x = 0; $x < $this->thumbnail_width; $x++) {
							for ($y = 0; $y < $this->thumbnail_height; $y++) {
								$AlphaChannelPixel = $this->phpthumb_functions->GetPixelColor($img_alpha_mixdown_dither, $x, $y);
		...
		if ($gdimg_flatten_temp = $this->phpthumb_functions->ImageCreateFunction($this->thumbnail_width, $this->thumbnail_height)) {

			$this->config_background_hexcolor = ($this->bg ? $this->bg : $this->config_background_hexcolor);
			if (!$this->phpthumb_functions->IsHexColor($this->config_background_hexcolor)) {
				return $this->ErrorImage('Invalid hex color string "'.$this->config_background_hexcolor.'" for parameter "bg"');
			}
			$background_color = $this->phpthumb_functions->ImageHexColorAllocate($this->gdimg_output, $this->config_background_hexcolor);
		...
		if ($this->fltr && is_array($this->fltr)) {
			...
			foreach ($this->fltr as $filtercommand) {
				...
				switch ($command) {
					...
					case 'clr': // Colorize
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping Colorize() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							break;
						}
						@list($amount, $color) = explode('|', $parameter);
					...
					case 'sep': // Sepia
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping Sepia() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							break;
						}
						@list($amount, $color) = explode('|', $parameter);
					...
					case 'rcd': // ReduceColorDepth
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping ReduceColorDepth() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							break;
						}
						@list($colors, $dither) = explode('|', $parameter);
						$colors = ($colors                ?  (int) $colors : 256);
					...
					case 'hist': // Histogram overlay
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping HistogramOverlay() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							break;
						}
					...
					case 'drop': // DropShadow
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping DropShadow() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						$this->is_alpha = true;
					...
					case 'mask': // Mask cropping
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping Mask() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							return false;
						}
					...
					case 'elip': // Elipse cropping
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping Elipse() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						$this->is_alpha = true;
					...
					case 'ric': // RoundedImageCorners
						if ($this->phpthumb_functions->gd_version() < 2) {
							$this->DebugMessage('Skipping RoundedImageCorners() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						...
						if (@is_readable($filename) && ($fp_watermark = @fopen($filename, 'rb'))) {
							...
							if ($img_watermark = $this->ImageCreateFromStringReplacement($WatermarkImageData)) {
								...
								if ($underlay) {
									...
									if ($img_watermark_resized = $this->phpthumb_functions->ImageCreateFunction(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output))) {
										...
										if ($img_source_resized = $this->phpthumb_functions->ImageCreateFunction($resized_x, $resized_y)) {
											...
											$this->DebugMessage('$this->phpthumb_functions->ImageCreateFunction('.$resized_x.', '.$resized_y.')', __FILE__, __LINE__);
										}
										ImageDestroy($img_watermark_resized);
									} else {
										$this->DebugMessage('$this->phpthumb_functions->ImageCreateFunction('.ImageSX($this->gdimg_output).', '.ImageSY($this->gdimg_output).')', __FILE__, __LINE__);
									}

								} else { // overlay

									if ($img_watermark_resized = $this->phpthumb_functions->ImageCreateFunction($resized_x, $resized_y)) {
										ImageAlphaBlending($img_watermark_resized, false);
										ImageSaveAlpha($img_watermark_resized, true);
										$this->ImageResizeFunction($img_watermark_resized, $img_watermark, 0, 0, 0, 0, ImageSX($img_watermark_resized), ImageSY($img_watermark_resized), ImageSX($img_watermark), ImageSY($img_watermark));
						...
						if (@is_readable($filename)) {
							if ($img_watermark = $this->ImageCreateFromFilename($filename)) {
								...
								if (eregi('^([0-9\\.\\-]*)x([0-9\\.\\-]*)$', $alignment, $matches)) {
									...
									$scale = $this->phpthumb_functions->ScaleToFitInBox(ImageSX($img_watermark), ImageSY($img_watermark), $watermark_max_width, $watermark_max_height, true, true);
									$this->DebugMessage('Scaling watermark by a factor of '.number_format($scale, 4), __FILE__, __LINE__);
									if (($scale > 1) || ($scale < 1)) {
										if ($img_watermark2 = $this->phpthumb_functions->ImageCreateFunction($scale * ImageSX($img_watermark), $scale * ImageSY($img_watermark))) {
					...
					case 'blur': // Blur
						...
						if ($this->phpthumb_functions->gd_version() >= 2) {
							...
							$this->DebugMessage('Skipping Blur() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							//return false;
						}
						break;
					...
					case 'size': // Resize
						...
						if ($stretch) {
							$scale_x = $this->phpthumb_functions->ScaleToFitInBox(ImageSX($this->gdimg_output), ImageSX($this->gdimg_output), $newwidth,  $newwidth,  true, true);
							$scale_y = $this->phpthumb_functions->ScaleToFitInBox(ImageSY($this->gdimg_output), ImageSY($this->gdimg_output), $newheight, $newheight, true, true);
						} else {
							$scale_x = $this->phpthumb_functions->ScaleToFitInBox(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output), $newwidth, $newheight, true, true);
							$scale_y = $scale_x;
						}
						...
						if (($scale_x > 1) || ($scale_x < 1) || ($scale_y > 1) || ($scale_y < 1)) {
							if ($img_temp = $this->phpthumb_functions->ImageCreateFunction(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output))) {
								...
								if ($this->gdimg_output = $this->phpthumb_functions->ImageCreateFunction($scale_x * ImageSX($img_temp), $scale_y * ImageSY($img_temp))) {
					...
					case 'usm': // UnSharpMask sharpening
						...
						if ($this->phpthumb_functions->gd_version() >= 2.0) {
							...
							$this->DebugMessage('Skipping unsharp mask because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						break;
					...
					case 'stc': // Source Transparent Color
						...
						if (!$this->phpthumb_functions->IsHexColor($hexcolor)) {
	...
	function MaxFileSize() {
		if ($this->phpthumb_functions->gd_version() < 2) {
			$this->DebugMessage('Skipping MaxFileSize() because gd_version is "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
			return false;
		}
		if ($this->maxb > 0) {
			switch ($this->thumbnailFormat) {
...
//echo $this->thumbnailCropH.'<hr>';
		...
		if ($this->zc && $this->w && $this->h) {
			...
			if ($this->aoe) {
				...
				$maxwidth  = $this->phpthumb_functions->nonempty_min($this->w, $this->thumbnailCropW, $this->config_output_maxwidth);
				$maxheight = $this->phpthumb_functions->nonempty_min($this->h, $this->thumbnailCropH, $this->config_output_maxheight);
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	function CreateGDoutput() {
		...
		$this->gdimg_output = $this->phpthumb_functions->ImageCreateFunction($this->thumbnail_width, $this->thumbnail_height);
		...
		ImageSaveAlpha($this->gdimg_output, true);
		if ($this->is_alpha && $this->phpthumb_functions->gd_version() >= 2) {

			ImageAlphaBlending($this->gdimg_output, false);
			$output_full_alpha = $this->phpthumb_functions->ImageColorAllocateAlphaSafe($this->gdimg_output, 255, 255, 255, 127);
	...
	function SetOrientationDependantWidthHeight() {
		...
		if ($this->source_height > $this->source_width) {
			// portrait
			$this->w = $this->phpthumb_functions->OneOfThese($this->wp, $this->w, $this->ws, $this->wl);
			$this->h = $this->phpthumb_functions->OneOfThese($this->hp, $this->h, $this->hs, $this->hl);
		} elseif ($this->source_height < $this->source_width) {
			// landscape
			$this->w = $this->phpthumb_functions->OneOfThese($this->wl, $this->w, $this->ws, $this->wp);
			$this->h = $this->phpthumb_functions->OneOfThese($this->hl, $this->h, $this->hs, $this->hp);
		} else {
			// square
			$this->w = $this->phpthumb_functions->OneOfThese($this->ws, $this->w, $this->wl, $this->wp);
			$this->h = $this->phpthumb_functions->OneOfThese($this->hs, $this->h, $this->hl, $this->hp);
		}
	...
	function ExtractEXIFgetImageSize() {
		...
		if ($this->phpthumb_functions->version_compare_replacement(phpversion(), '4.2.0', '>=') && function_exists('exif_read_data')) {
		...
		if (function_exists('exif_thumbnail') && ($this->getimagesizeinfo[2] == 2)) {
			...
			if ($this->phpthumb_functions->version_compare_replacement(phpversion(), '4.3.0', '>=')) {
	...
	function SetCacheFilename() {
		...
		if (@$_SERVER['HTTP_REFERER'] && $this->config_nooffsitelink_enabled) {
			$parsed_url1 = @$this->phpthumb_functions->ParseURLbetter(@$_SERVER['HTTP_REFERER']);
			$parsed_url2 = @$this->phpthumb_functions->ParseURLbetter('http://'.@$_SERVER['HTTP_HOST']);
		...
		if ($this->md5s) {
			...
			$this->cache_filename .= '_dat'.intval($this->phpthumb_functions->filedate_remote($this->src));
	...
	function SourceImageToGD() {
		...
		if ($this->ImageMagickThumbnailToGD()) {
			...
			if (!$this->gdimg_source) {
				...
				$this->DebugMessage('SourceImageToGD.ImageCreateFromStringReplacement() failed with unknown image type "'.substr($this->rawImageData, 0, 4).'" ('.$this->phpthumb_functions->HexCharDisplay(substr($this->rawImageData, 0, 4)).')', __FILE__, __LINE__);
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.$this->phpthumb_functions->HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
			}

		} elseif (!$this->gdimg_source && $this->sourceFilename) {

			if ($this->md5s && ($this->md5s != $this->phpthumb_functions->md5_file_safe($this->sourceFilename))) {
				return $this->ErrorImage('$this->md5s != md5(sourceFilename)'."\n".'"'.$this->md5s.'" != '."\n".'"'.$this->phpthumb_functions->md5_file_safe($this->sourceFilename).'"');
			}
		...
		if (!$this->gdimg_source) {
			...
			switch (@$this->getimagesizeinfo[2]) {
				case 6:
					...
					$phpthumb_bmp = new phpthumb_bmp();
					$this->gdimg_source = $phpthumb_bmp->phpthumb_bmp2gd($this->rawImageData, ($this->phpthumb_functions->gd_version() >= 2.0));
					unset($phpthumb_bmp);
					if ($this->gdimg_source) {
			...
			if (!$this->gdimg_source) {
				...
				return $this->ErrorImage('Unknown image type identified by "'.substr($HeaderFourBytes, 0, 4).'" ('.$this->phpthumb_functions->HexCharDisplay(substr($HeaderFourBytes, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	function phpThumbDebug($level='') {
		...
		foreach ($FunctionsExistance as $functionname) {
			$DebugOutput[] = 'builtin_function_exists('.$functionname.')'.str_repeat(' ', 23 - strlen($functionname)).' = '.$this->phpThumbDebugVarDump($this->phpthumb_functions->builtin_function_exists($functionname));
		}
		$DebugOutput[] = '';
		...
		$exif_info = $this->phpthumb_functions->exif_info();
	...
	function ErrorImage($text, $width=0, $height=0, $forcedisplay=false) {
		...
		$this->setOutputFormat();
		if (!$this->thumbnailFormat || ($this->phpthumb_functions->gd_version() < 1)) {
		...
		$headers_file = '';
		$headers_line = '';
		if ($this->phpthumb_functions->version_compare_replacement(phpversion(), '4.3.0', '>=') && headers_sent($headers_file, $headers_line)) {
			...
			$background_color = $this->phpthumb_functions->ImageHexColorAllocate($gdimg_error, $this->config_error_bgcolor,   true);
			$text_color       = $this->phpthumb_functions->ImageHexColorAllocate($gdimg_error, $this->config_error_textcolor, true);
	...
	function ImageCreateFromStringReplacement(&$RawImageData, $DieOnErrors=false) {
		...
		if (!$this->phpthumb_functions->gd_version()) {
			if ($DieOnErrors) {
				...
				$this->DebugMessage('ImageCreateFromStringReplacement() failed: gd_version says "'.$this->phpthumb_functions->gd_version().'"', __FILE__, __LINE__);
				return false;
			}
		}
		if ($this->phpthumb_functions->gd_is_bundled()) {
		...
		switch (substr($RawImageData, 0, 3)) {
			...
			case "\x89".'PN':
				...
				break;
			default:
				$this->DebugMessage('ImageCreateFromStringReplacement() failed: unknown fileformat signature "'.$this->phpthumb_functions->HexCharDisplay(substr($RawImageData, 0, 3)).'"', __FILE__, __LINE__);
				return false;
				break;
		}
	...
	function ImageResizeFunction(&$dst_im, &$src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH) {
		...
		if ($this->phpthumb_functions->gd_version() >= 2.0) {
			if ($this->config_disable_imagecopyresampled) {
				return $this->phpthumb_functions->ImageCopyResampleBicubic($dst_im, $src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH);
			}
